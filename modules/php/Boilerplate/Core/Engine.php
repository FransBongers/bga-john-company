...
class Engine
{
  ...
  {
    if (!self::$tree instanceof \Bga\Games\JohnCompany\Boilerplate\Core\Engine\SeqNode) {
      self::$tree = new \Bga\Games\JohnCompany\Boilerplate\Core\Engine\SeqNode([], [self::$tree]);
      self::save();
    }
  }
  ...
  public static function buildTree($t)
  {
    ...
    $className = '\Bga\Games\JohnCompany\Boilerplate\Core\Engine\\' . ucfirst($type) . 'Node';
    unset($t['children']);
