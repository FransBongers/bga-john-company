...
class Bar {
    ...
    setup(gamedatas) {
        ...
        bar.addEventListener('click', (event) => this.onIconClick(event));
    }
    onIconClick(event) {
...
const createFamilyMember = (familyId, familyMemberId, extraClasses) => {
    ...
    elt.insertAdjacentHTML('afterbegin', familyMemberSvgs$1[familyMemberNumber] ?? familyMemberSvgs$1[1]);
...
const tplPlayerCounters = ({ crownInGame, familyId, playerId, }) => {
...
class JocoPlayer {
    ...
    clearInterface() { }
    updatePlayer(gamedatas) {
        this.updatePlayerPanel(gamedatas);
    }
    setupPlayer(gamedatas) {
        this.setupPlayerPanel(gamedatas);
    }
    setupPlayerPanel(gamedatas) {
        const playerGamedatas = gamedatas.players[this.playerId];
        const node = document.querySelector(`#player_board_${this.playerId} .player-board-game-specific-content`);
        if (!node) {
            return;
        }
        const familyId = this.familyId === CROWN
        ...
        counters.forEach((counter) => {
            ...
            this.ui[counter] = document.getElementById(`joco-${counter}-${this.playerId}`);
        });
        this.updatePlayerPanel(gamedatas);
    }
    updatePlayerPanel(gamedatas) {
...
let Interaction$1 = class Interaction {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
    ...
    addPlayerButton({ id, text, playerId, callback, extraClasses, }) {
    ...
    addPrimaryActionButton({ id, text, callback, extraClasses, }) {
        if ($(id)) {
            return;
        }
    ...
    addSecondaryActionButton({ id, text, callback, extraClasses, }) {
        if ($(id)) {
            return;
        }
        this.game.bga.statusBar.addActionButton(text, callback, {
    ...
    addCancelButton({ callback, extraClasses, } = {}) {
    ...
    addDangerActionButton({ id, text, callback, extraClasses, }) {
        if ($(id)) {
            return;
        }
        this.game.bga.statusBar.addActionButton(text, callback, {
    ...
    addPassButton(optionalAction, text) {
        if (optionalAction) {
            this.addSecondaryActionButton({
                id: 'pass_btn',
                text: text ? _(text) : _('Pass'),
                callback: () => this.game.bga.actions.performAction('actPassOptionalAction'),
            });
        }
    }
    addUndoButtons({ previousSteps, previousEngineChoices, }) {
    ...
    async performAction(actionName, args) {
        return await this.game.bga.actions.performAction('actTakeAtomicAction', {
            actionName,
...
const addCancelButton = (props = {}) => {
    Interaction$1.use().addCancelButton(props);
};
const addConfirmButton = (callback) => {
    Interaction$1.use().addConfirmButton(callback);
};
const addDangerActionButton = (props) => {
    Interaction$1.use().addDangerActionButton(props);
};
const addPassButton = (optionalAction, text) => Interaction$1.use().addPassButton(optionalAction, text);
const addPlayerButton = (props) => Interaction$1.use().addPlayerButton(props);
const addPrimaryActionButton = (props) => Interaction$1.use().addPrimaryActionButton(props);
const addSecondaryActionButton = (props) => Interaction$1.use().addSecondaryActionButton(props);
const addUndoButtons = (props) => {
    Interaction$1.use().addUndoButtons(props);
};
const clearPossible = () => {
    Interaction$1.use().clearPossible();
};
const updatePageTitle = (text, args = {}, nonActivePlayers = false) => Interaction$1.use().clientUpdatePageTitle(text, args, nonActivePlayers);
const formatStringRecursive = (log, args) => {
    return Interaction$1.use().formatStringRecursive(log, args);
};
...
const onClick = (node, callback, temporary = true) => {
    let element = typeof node === 'string' ? document.getElementById(node) : node;
    Interaction$1.use().onClick(element, callback, temporary);
};
const setSelected = (node) => {
    let element = typeof node === 'string' ? document.getElementById(node) : node;
    Interaction$1.use().setSelected(element);
};
const performAction = (actionName, args) => {
    Interaction$1.use().clearPossible();
    Interaction$1.use().performAction(actionName, args);
};
...
const getCompanyBalanceConfig = (balance) => {
...
const getGroupPosition = (top, left, index, rowSize) => {
...
const getRegimentGroupPosition = (top, left, index, rowSize) => {
    const row = Math.floor(index / rowSize);
    const column = index % 4;
    return {
        top: top + 46 * row,
...
const getRegimentPosition = (location, index, exhausted) => {
    switch (location) {
        case BOMBAY_ARMY:
...
const getCourtOfDirectorsPosition = (index) => getGroupPosition(555, 243, index, 4);
const getOfficersInTrainingPosition = (index) => getGroupPosition(602, 692, index, 3);
const getWriterPosition = (location, index) => {
    switch (location) {
        case BOMBAY_WRITERS:
...
const getStockExchangePosition = (position) => {
    switch (position) {
...
const getShipPosition = (sea, index) => {
...
const POWER_TOKEN_POSITIONS = [
    { top: 72, left: 276 },
    { top: 115, left: 276 },
    { top: 159, left: 251 },
    { top: 159, left: 301 },
];
const ARMY_SELECT_POSITIONS = {
    [BOMBAY_ARMY]: { top: 21, left: 692 },
    [MADRAS_ARMY]: { top: 214, left: 692 },
    [BENGAL_ARMY]: { top: 407, left: 692 },
};
const COMPANY_DEBT_SELECT_POSITIONS = [
    { top: 157, left: 365.5 },
    { top: 157, left: 400 },
    { top: 157, left: 434 },
    { top: 157, left: 468.5 },
...
const SEA_ZONE_SELECT_POSITIONS = {
...
class Board {
    ...
    updateFamilyMembers(familyMembers) {
        familyMembers.forEach((familyMember) => {
            ...
            switch (location) {
                ...
                case BOMBAY_WRITERS:
                case MADRAS_WRITERS:
                    position = getWriterPosition(location, this.familyMembers[location].length);
                    this.familyMembers[location].push(familyMember);
                    break;
                case BENGAL_ARMY:
                case BOMBAY_ARMY:
                case MADRAS_ARMY:
                    position = getOfficerPosition(location, this.familyMembers[location].length);
                    this.familyMembers[location].push(familyMember);
                    break;
                case STOCK_EXCHANGE_2:
                case STOCK_EXCHANGE_3_LEFT:
    ...
    async moveFamilyMember({ familyMember, index = 0, }) {
        await Interaction$1.use().wait(index * 200);
        const fromRect = this.ui.familyMembers[familyMember.id].getBoundingClientRect();
        this.updateFamilyMembers([familyMember]);
        await this.game.animationManager.slideIn(this.ui.familyMembers[familyMember.id], this.ui.familyMembers[familyMember.id]);
    }
    async moveFamilyMemberBetweenLocations(familyMember, to) {
        const from = familyMember.location;
        familyMember.location = to;
        if (this.familyMembers[familyMember.location] &&
            this.familyMembers[familyMember.location].some((memberInLocation) => memberInLocation.id === familyMember.id)) {
            return;
        }
        let promises = [];
        if (this.familyMembers[from]) {
            const remainingFamilyMembers = this.familyMembers[from].filter((member) => member.id !== familyMember.id);
            this.familyMembers[from] = [];
            promises = remainingFamilyMembers.map((member) => this.moveFamilyMember({ familyMember: member }));
        }
    ...
    async placeFamilyMembers(familyMembers, fromElement) {
        const promises = familyMembers.map(async (familyMember, index) => {
            ...
            const player = PlayerManager.getInstance().getPlayerForFamily(familyMember.familyId);
            await Interaction$1.use().wait(index * 200);
    ...
    updatePowerTokens(gamedatas) {
        gamedatas.powerTokens.forEach((token, index) => {
            setAbsolutePosition(this.ui.containers.powerTokens[token], BOARD_SCALE, POWER_TOKEN_POSITIONS[index]);
        });
    }
    async movePawn(type, value) {
    ...
    async moveRegimentBetweenArmies(regiment, from) {
        if (this.armies.regiments[regiment.location].some((regimentInLocation) => regimentInLocation.id === regiment.id)) {
            return;
        }
        const remainingRegiments = this.armies.regiments[from].filter((piece) => piece.id !== regiment.id);
        this.armies.regiments[from] = [];
        const promises = remainingRegiments.map((remaining) => this.moveRegiment(remaining));
        promises.push(this.moveRegiment(regiment));
        await Promise.all(promises);
    }
    async moveShip({ ship, index = 0, from, }) {
        await Interaction$1.use().wait(index * 200);
        const fromRect = this.ui.ships[ship.id].getBoundingClientRect();
        const fromIndex = this.seas[from].findIndex((shipInOldZone) => shipInOldZone?.id === ship.id);
        this.placeShip(ship);
        if (fromIndex >= 0) {
    ...
    async removeShip(shipId, seaZone) {
        this.ui.ships[shipId].remove();
        const fromIndex = this.seas[seaZone].findIndex((shipInOldZone) => shipInOldZone?.id === shipId);
        if (fromIndex >= 0) {
            this.seas[seaZone][fromIndex] = null;
        }
    }
    updateOtherShip(ship, type) {
        this.ui.ships[ship.id].setAttribute('data-type', type);
        ship.type = type;
        ship.name = type === 'ExtraShip' ? _('Extra Ship') : _('Company Ship');
        return ship;
    }
    shipAlreadyInZone(shipId, location) {
        return this.seas[location].some((shipInLocation) => shipInLocation?.id === shipId);
    }
    async placeShip(ship, fromElement) {
        ...
        if ([SOUTH_INDIAN, WEST_INDIAN, EAST_INDIAN].includes(location)) {
            ...
            const nullIndex = this.seas[location].findIndex((pos) => pos === null);
...
class NotificationManager {
    ...
    async pay(playerId, amount) {
        await Interaction$1.use().wait(1);
        ...
        const promises = Array.from(Array(amount).keys()).map(async (_, index) => {
            await Interaction$1.use().wait(index * 150);
    ...
    async notif_elephantMarch(notif) {
        ...
        await Interaction$1.use().wait(500);
    }
    ...
    async notif_gainCash(notif) {
        ...
        const logPound = document.querySelector('#pagemaintitletext .joco_pound');
        const promises = Array.from(Array(amount).keys()).map(async (_, index) => {
            await Interaction$1.use().wait(index * 100);
            const element = document.createElement('div');
            element.classList.add('log_token');
            element.classList.add('joco_pound');
            element.classList.add('animation');
    ...
    async notif_transferPromiseCubes(notif) {
        ...
        const promises = Array.from(Array(Math.abs(amount)).keys()).map(async (_, index) => {
            Interaction$1.use().wait(index * 250);
...
class Chairman {
    ...
    updateInterfaceInitialStep() {
        ...
        const board = Board.getInstance();
        const interaction = Interaction$1.use();
    ...
    async handleDebtClick(value, requiresVote) {
        if (requiresVote) {
            ...
            const noVoteValue = noVoteOptionCount > 0
...
const tplLogTokenIcon = (type) => `<div class="log-token joco-icon" data-icon="${type}"></div>`;
...
class CrownChairmanRequestAllocation {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class CrownChairmanRequestDebtAdvancement {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class CrownManagerOfShippingBuyCompanyShips {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class CrownManagerOfShippingFitShips {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class CrownManagerOfShippingLeaseExtraShips {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class CrownManagerOfShippingPlaceShips {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class CrownManagerOfShippingUnfittedShipOptions {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
    ...
    updateInterfaceInitialStep() {
        this.game.clearPossible();
        this.updatePageTitle();
        if (this.args.ship.id === this.ship.id &&
...
class DirectorOfTradeSpecialEnvoy {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class DirectorOfTradeSpecialEnvoySuccess {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class DirectorOfTradeTransfers {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class DraftCard {
    ...
    handleClick(cardId) {
        if (this.selectedCards.includes(cardId)) {
            this.selectedCards = this.selectedCards.filter((selectedCardId) => selectedCardId !== cardId);
        }
        else {
...
class EventsInIndiaCrisisDefense {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
...
class PresidencyTradeFillOrders {
    constructor(game) {
        this.game = game;
    }
    static create(game) {
    ...
    updateInterfaceInitialStep() {
        ...
        const placedWriters = this.filledOrders.map((filledOrder) => filledOrder.filledBy);
        const availableWriters = this.args.writers.filter((writer) => !placedWriters.includes(writer.id));
    ...
    getAvailableOrderIds() {
        ...
        Object.values(this.args.orders).forEach((order) => {
            ...
            const connectedOrderHasBeenFilled = filledOrderIds.some((orderId) => staticOrder.connectedOrders.includes(orderId));
...
class Game {
    constructor(bga) {
        ...
        this.bga.userPreferences.onChange = (prefId, prefValue) => this.onUserPrefChanged(prefId, prefValue);
    }
    onScreenWidthChange() {
        this.updateLayout();
    }
    overrideOnPlaceLogOnChannel() {
    ...
    overrideAddToLog() {
        const originalAddToLog = this.bga.notifications.game.notifqueue.addToLog.bind(this.bga.notifications.game.notifqueue);
        this.bga.notifications.game.notifqueue.addToLog = (input) => {
    ...
    overrideSetLoader() {
        const originalSetLoader = this.bga.gameui.setLoader.bind(this.bga.gameui);
        this.bga.gameui.setLoader = (value, max) => {
    ...
    overrideUpdatePlayerOrdering() {
        ...
        this.bga.gameui.updatePlayerOrdering = () => {
            ...
            this.playerOrder.forEach((playerId) => {
                const playerBoard = document.getElementById(`overall_player_board_${playerId}`);
    ...
    onEnteringState(stateName, args) {
        ...
        const activePlayerIds = args.args?.activePlayerIds;
        const playerIsActiveAndStateExists = this.bga.players.isCurrentPlayerActive() &&
    ...
    onUserPrefChanged(prefId, prefValue) {
        switch (prefId) {
        }
    }
    clearInterface() {
    ...
    onAddingNewUndoableStepToLog(notif) {
        ...
        if (this.gamedatas.gamestate.args.previousSteps?.includes(Number(stepId))) {
